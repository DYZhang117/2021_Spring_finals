...
def getTotalDailyVaccinated(stateVaccinations: pd.DataFrame) -> pd.DataFrame:
    ...
    Updated On
    2021-02-15                      NaN                       NaN
    2021-02-16                  12559.0                    3388.0
    2021-02-17                8646599.0                 2812416.0
    2021-02-18                  30742.0                  214215.0
    """
...
def hypothesis3(testingData3: pd.DataFrame):
    ...
    covis_top_3 = statewise_vaccination.groupby(['Updated On']).apply(rank_, 'CoviShield (Doses Administered)', 3) \
        .reset_index().drop('level_1', axis=1)
    covis_top = covis_top_3.drop_duplicates('Updated On', keep='first')

    covx_top_3 = statewise_vaccination.groupby(['Updated On']).apply(rank_, 'Covaxin (Doses Administered)', 3) \
        .reset_index().drop('level_1', axis=1)
...
if __name__ == '__main__':
    ...
    stateTesting = pd.read_csv("https://api.covid19india.org/csv/latest/statewise_tested_numbers_data.csv") # Number of tests conducted in each state, ventilators ,hospital bed occupany reported in state bulletins
    India_positives = pd.read_csv('https://api.covid19india.org/csv/latest/case_time_series.csv') # India level timeseries for Confirmed, Recovered and Deceased cases
